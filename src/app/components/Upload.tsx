...
import { Camera, X, ArrowLeft, Upload as UploadIcon, CheckCircle2 } from 'lucide-react';
...
export default function Upload() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const imageList = photos.length > 0 ? photos : ['https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=800'];
    ...
    addUserProduct({
      ...
      trades: 0,
      city: 'Chimbote',
      sellerName: userName.split(' ')[0] + ' ' + (userName.split(' ')[1]?.[0] || '') + '.',
  ...
  const slotLabels = ['Frente', 'Lateral', 'Trasera', 'Detalle 1', 'Detalle 2', 'En uso'];
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-4">
          <button onClick={() => navigate('/feed')} className="text-[#0F3460] hover:bg-slate-100 p-2 rounded-lg">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <h1 className="text-xl font-bold text-[#0F3460]">{t('Publicar Producto')}</h1>
        </div>
      </div>
      ...
      <div className="px-4 py-6 max-w-2xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-md">
            ...
            >
              <UploadIcon className={`w-8 h-8 ${draggingOver ? 'text-[#16A085]' : 'text-slate-400'}`} />
              ...
              {Array.from({ length: maxPhotos }, (_, i) => {
                ...
                return (
                  <div key={i} className="relative aspect-square">
                    ...
                    <p className="text-[10px] text-slate-400 text-center mt-1">{slotLabels[i]}</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-md space-y-4">
            <h2 className="font-bold text-[#0F3460] mb-4">{t('Información del Producto')}</h2>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('Título del producto')}</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('Categoría')}</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('Estado del producto')}</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('Descripción')}</label>
              <textarea
          ...
          >
            {photos.length < 1
              ? (language === 'en' ? 'Add at least 1 photo' : 'Agrega al menos 1 foto')
              : t('Publicar Producto')}
          </button>
        </form>
      </div>
