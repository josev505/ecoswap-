...
const mockProducts = [
  ...
  },
  {
    ...
    rating: 4.8,
    trades: 13,
    image: "https://images.unsplash.com/photo-1604930008933-3e4b99d993b2?w=400",
    condition: "Usado",
    city: "Trujillo",
    sellerName: "Sergio P.",
    sellerInitials: "SP",
...
export default function Feed() {
  ...
  const allProducts = useMemo(() => {
    ...
    return [
      ...userMapped,
      ...mockProducts.filter(
        (mp) => !userProducts.find((up) => up.id === mp.id),
      ),
    ];
  }, [userProducts]);
  ...
  const [conditionDropdownPos, setConditionDropdownPos] = useState({
    top: 0,
    left: 0,
  });
  const [ratingDropdownPos, setRatingDropdownPos] = useState({
    top: 0,
    left: 0,
  });
  ...
  const filteredProducts = useMemo(() => {
    return allProducts.filter((product) => {
      ...
      const matchesRating =
        !selectedRating || product.rating >= getMinRating(selectedRating);
      return (
        matchesSearch &&
        matchesCategory &&
        matchesCondition &&
          ...
          <div ref={topBarContentRef}>
            ...
            <div className="px-4 pb-4 max-w-7xl mx-auto flex items-center gap-3">
              ...
              <div className="relative flex-shrink-0">
                ...
                >
                  <span className="flex items-center gap-1">
                    <MapPin className="w-3 h-3" /> {t("Ubicación")}{" "}
                    <ChevronDown className="w-3 h-3" />
                  </span>
                  <span className="text-xs font-normal opacity-80">
                    {selectedCity || t("Todas")}
                  </span>
                </button>
                {showCityDropdown && (
                  <div
              ...
              <div className="relative flex-shrink-0">
                ...
                >
                  <span className="flex items-center gap-1">
                    {t("Estado")} <ChevronDown className="w-4 h-4" />
                  </span>
                  <span className="text-xs font-normal opacity-80">
                    {t(selectedCondition)}
                  </span>
                ...
                {showConditionDropdown && (
                  <div
                    ...
                    className="fixed bg-white border border-slate-200 rounded-xl shadow-lg py-2 min-w-[150px]"
                    style={{
                      top: conditionDropdownPos.top,
                      left: conditionDropdownPos.left,
                      zIndex: 9999,
                    }}
                  >
              ...
              <div className="relative flex-shrink-0">
                ...
                >
                  <span className="flex items-center gap-1">
                    ⭐ {t("Valoración")} <ChevronDown className="w-3 h-3" />
                  </span>
                  <span className="text-xs font-normal opacity-80">
                    {selectedRating || t("Todas")}
                  </span>
                </button>
                {showRatingDropdown && (
                  <div
      ...
      <div className="px-4 py-6 max-w-7xl mx-auto">
        ...
        <div className="bg-[#E8F5F0] rounded-[8px] p-[12px] mb-6">
          <p className="text-[#0D3B4F] text-[11px]">
            Hypothesis: Showing the seller's photo and allowing users to save
            favorites builds greater trust and interaction.
          </p>
        </div>
            ...
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <div className="relative h-48 bg-slate-200">
                  ...
                  >
                    <Heart
                      className="w-4 h-4"
                      style={{
                        color: favorites.has(product.id)
                          ? "#16A085"
                          : "#94a3b8",
                        fill: favorites.has(product.id) ? "#16A085" : "none",
                      }}
                    />
                  </button>
                ...
                <div className="p-4">
                  ...
                  <div className="mb-3">
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-xs font-semibold border ${getConditionColor(product.condition)}`}
                    >
                      {t(product.condition)}
                    </span>
                  </div>
